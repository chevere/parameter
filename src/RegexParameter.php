...
final class RegexParameter implements RegexParameterInterface
{
    use ParameterTrait;
    ...
    public function withRegex(RegexInterface $regex): RegexParameterInterface
    {
        $new = clone $this;
        $new->regex = $regex;
    ...
    public function withDefault(string $value): RegexParameterInterface
    {
        if ($this->regex->match($value) === []) {
            throw new InvalidArgumentException(
                (string) message(
    ...
    public function assertCompatible(RegexParameterInterface $parameter): void
    {
